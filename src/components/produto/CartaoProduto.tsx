...
export interface ICartaoProdutoProps {
  produto: Produto;
}

export const CartaoProduto = (props: ICartaoProdutoProps) => {
  const { adicionar } = useCarrinho();

  const { descricao, imagem, nome, preco } = props.produto;
  return (
    <div className="flex flex-col w-72 bg-zinc-900">
      <div className="relative w-72 h-52 ">
        <Image src={imagem} alt={nome} fill className="object-cover" />
      </div>
      <div className="flex-1 flex flex-col gap-4 p-5 text-white">
